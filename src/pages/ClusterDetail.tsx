...
export default function ClusterDetail({ clusterId, onBack }: Props) {
  ...
  const selectedMembers = cluster.members.filter(m => selectedForCompare.has(m.id) && m.codeLines && m.codeLines.length > 0);
    ...
    <div>
      ...
      {activeTab === 'compare' && (
        <div>
          ...
          {/* Implementation Selection Table with Checkboxes */}
          {cluster.members.length > 0 && (
            <div className="sf-card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e5e5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h4 className="sf-section-title" style={{ margin: 0 }}>Select Implementations to Compare</h4>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  {selectedForCompare.size > 0 && (
                    <button className="sf-btn sf-btn-sm" onClick={() => setSelectedForCompare(new Set())} style={{ fontSize: 11 }}>
                      Clear Selection
                    </button>
                  )}
                  <span style={{ fontSize: 12, color: selectedForCompare.size >= 2 ? '#2e844a' : '#706e6b', fontWeight: 600 }}>
                    {selectedForCompare.size} selected
                    {selectedForCompare.size < 2 && ' — select at least 2'}
                  </span>
                </div>
              </div>
              <table className="sf-table" style={{ marginBottom: 0 }}>
                ...
                <tbody>
                  {cluster.members.map(m => {
                    ...
                    const isChecked = selectedForCompare.has(m.id);
                    return (
                      <tr key={m.id} onClick={() => toggleCompareSelection(m.id)} style={{ background: isChecked ? '#f0f7ff' : undefined }}>
                        <td style={{ textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() => toggleCompareSelection(m.id)}
                            style={{ width: 16, height: 16, accentColor: '#0176d3', cursor: 'pointer' }}
                          />
                        </td>
                        <td style={{ fontFamily: 'monospace', fontSize: 12, color: '#0176d3', fontWeight: isChecked ? 600 : 400 }}>{m.name.split('(')[0]}()</td>
                        <td>{m.surface}</td>
                        <td>{m.loc}</td>
          ...
          {selectedMembers.length >= 2 && (() => {
            ...
            <div className="sf-card" style={{ marginTop: 16, overflow: 'hidden' }}>
              ...
              {/* Column Headers */}
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${selectedMembers.length}, 1fr)`, borderBottom: '2px solid #e5e5e5' }}>
                {selectedMembers.map((m, colIdx) => {
                  ...
                  return (
                    <div key={m.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 600, color: '#0176d3', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {m.name.split('(')[0]}()
                        </span>
                        <span style={{ padding: '1px 8px', borderRadius: 4, fontSize: 10, fontWeight: 600, background: bc.bg, color: bc.color, flexShrink: 0, marginLeft: 6 }}>
                          {m.badge}
                        </span>
                      </div>
                      <div style={{ fontSize: 10, color: '#706e6b' }}>
                        {m.loc} lines • {m.similarity}% similar • {m.owner}
                      </div>
                    </div>
                  );
                })}
          ...
          {selectedMembers.length < 2 && selectedForCompare.size > 0 && (
            <div className="sf-card" style={{ padding: '32px 20px', marginTop: 16, textAlign: 'center' }}>
              <p style={{ color: '#706e6b', fontSize: 13 }}>
                Select at least 2 implementations with code data above to see the side-by-side comparison.
              </p>
            </div>
          )}
